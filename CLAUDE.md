...
4. **Hyvä-compatible — the storefront is 100% vanilla JS.** No RequireJS, Knockout or jQuery anywhere on the storefront, including checkout. Admin can use standard Magento UI Components (Knockout there is fine — Hyvä doesn't touch admin).
   An exception was briefly added for a LUMA checkout adapter and is now **withdrawn**: checkout steps are detected from `window.location.hash`, which Magento's `step-navigator` sets to the active step code, so no theme-specific JS stack is needed. Two earlier attempts (a jsLayout component and a RequireJS mixin) were discarded — see PROGRESS.md.
   **Two hard-won rules for storefront JS in this module:**
   - **Never emit an inline `<script>`.** Magento's CSP blocks inline scripts on checkout, which silently left the beacon config undefined there and killed every checkout event while product pages kept working. Config travels as a `data-` attribute read by the external beacon file.
   - A checkout that cannot be observed from the hash (Hyvä Checkout) calls the beacon's public `window.aavirbhavaAdsAnalytics.track(eventType)` instead — see README.md. An adapter must contain no tracking logic of its own.
