...
- `customer_id` and `order_id` are **never** accepted in the ingest payload. The consumer resolves `customer_id` from the authenticated session (if any) and links `order_id` server-side via the `sales_order_place_after` observer — not from anything the frontend sends.
