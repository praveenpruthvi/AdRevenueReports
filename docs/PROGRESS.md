...
### Upload UI CSS, and every configured platform can now upload spend data (2026-09-20)

Two more rounds of direct feedback on the upload feature above, both landed same-day.

**CSS.** The whole "Ad Spend Data" section (and, it turned out, the "Return on Ad Spend" section next to it) had no padding anywhere — text and table cells touched the section's edges, and the skipped-rows warning sat directly against the row-count line above it via a bare `<br>`. Root cause: `.admin__fieldset-wrapper-content` only pads a direct-child `.admin__fieldset` (a real Magento config-form fieldset) — confirmed by reading core's own `forms/_fields.less` — and nothing on this custom page uses that markup, so there was never any padding to inherit. `.aavirbhava-ads-charts__content` (the funnel/traffic chart section) already had its own explicit `padding: 20px`, which is exactly why that section looked correct while the two added later did not. Every section's content area now gets the same treatment, table cells get real padding, notes get margin before/after, and the warning is a block-level element with its own spacing instead of a bare `<br>`.

**Only google and meta could upload — a real, not cosmetic, limitation.** `AdSpendProviderPool` only knew about whatever `etc/di.xml` explicitly listed, which was google and meta. But the store already classifies traffic for `bing`, `tiktok` and `reddit` too — "Paid Platform / Click-ID Map" (admin-editable, `Model\Config\TrafficClassificationConfig::getPlatformMap()`) already lists all five — and `CsvAdSpendProvider` is completely generic, so there was no actual reason those three couldn't upload a spend CSV the same way. Fixed by having `AdSpendProviderPool` fall back to the generic CSV provider for any platform in that admin-editable list that has no explicit `di.xml` entry, rather than requiring one per platform. An explicit `di.xml` entry still wins over the fallback — a client project registering a real, non-CSV provider for a specific platform is unaffected. This means a platform an admin adds to that list *after* this module was installed also gets CSV-based spend upload automatically, with no developer involvement ever — the same "no platform-specific code" principle (CLAUDE.md #2) already applied to traffic classification, now applied to ad spend too.

**Checked the "1 rows in this file could not be read" warning directly, per a follow-up question.** Traced it to two deliberately malformed rows in the module's own SEEDED demo data — `google.csv` had `2026-09-17,clearance,not_a_number` (bad spend value), `meta.csv` had `bad-date,brand_awareness,20.00` (bad date) — planted during earlier verification specifically to prove the fail-open parsing works. The log confirmed the exact reason for each; this was the feature working correctly, not a defect. Regenerated both demo files without the planted bad rows, since that behaviour is now proven several times over (unit tests, a real end-to-end upload test, and this) and there is no reason to leave a permanent, confusing-looking warning in the demo data.

**While checking that, tested the spend-value parser against realistic real-world CSV export quirks** (surrounding whitespace, integers, zero, scientific notation, a leading currency symbol, a comma thousands separator). All handled correctly — nothing is silently misread — but the comma-thousands-separator case (`"1,234.56"`, which is what you get by default copying a formatted currency column out of Excel or Google Sheets) is a genuinely likely way a real merchant's upload gets a row rejected, and the format instructions did not call it out specifically (only "no currency symbol"). Left the validation strict — silently reinterpreting `1,234.56` as `1234.56` risks a wrong number going unnoticed, which is worse than a row being skipped with a clear reason — and added the explicit warning to both the admin note and the on-page upload instructions instead.

Verified through the real DI container: `getPlatformCodes()` now returns `[google, meta, bing, tiktok, reddit]`; the rendered upload table correctly lists and would accept uploads for all five. 8 new/updated unit tests (238 total green), 0 phpcs errors, 0 phpstan errors on the changed class.

### Ad-spend CSV upload, after "what if the admin can't reach the file server?" (2026-09-20)

The file-drop-only version of ad spend shipped at P4-T4/T5 assumed whoever manages ROAS data has server filesystem access. Pushed back on directly, correctly: not every admin does.

**Refactored row-validity parsing out of `CsvAdSpendProvider` first**, into a new shared `Model\AdSpendProvider\AdSpendCsvFile`. This was not optional scope creep — without it, the upload path and the read path would each have grown their own copy of "is this row valid", and the two would eventually disagree: a file that "uploads successfully" but then reads differently once saved would be a worse experience than the original problem. `CsvAdSpendProvider` itself shrank to just the missing-file short-circuit and date-range filtering over what `AdSpendCsvFile` parses.

**`Model\AdSpendProvider\AdSpendCsvUploader`** validates a file completely before writing anything: parses it through the same `AdSpendCsvFile` the read path uses, and refuses to save (with a plain-language reason) a file that yields zero valid rows — a wrong-format upload must not silently replace a platform's working spend data with an empty file. A file with *some* valid rows is accepted, and the response reports exactly how many rows were skipped and why. Deliberately does not use `\Magento\Framework\File\Uploader`: that class's rename-on-collision and directory-dispersion behaviour is built for the media-gallery case and actively fights the one thing this needs — a fixed filename per platform, always overwritten on re-upload, the same as replacing the file by hand.

**`Controller\Adminhtml\AdSpend\Upload`** is gated on `Aavirbhava_AdsAnalytics::config` — the same resource the module's settings use, since uploading spend data is closer to "changes what this module reports" than "views a report". `Block\Adminhtml\Dashboard::canManageAdSpendData()` checks that same resource before the template renders the form at all, so an admin who cannot submit it never sees it and finds out by a confusing rejection.

**The upload lives on the report page itself** (Reports > Ads Analytics, a new "Ad Spend Data" section under the ROAS table), one small form per registered platform (`AdSpendProviderPool::getPlatformCodes()`, extended with this method) rather than a shared dropdown — a merchant uploads one platform's export at a time, and a row-per-platform layout keeps which file is for which platform unambiguous without JavaScript. Each row shows the platform's current data (row count, last updated, any rows that couldn't be read) before the admin uploads a replacement.

**Verification had a real obstacle worth recording.** `is_uploaded_file()` — the one call that guards against a forged path being read as though it were a genuine upload — cannot be satisfied by any unit-test fixture; PHPUnit's own documentation says so. Confirmed the guard is real and active by hitting the real controller from a plain `tempnam()`-created file in a genuine (non-test) process first, which it correctly refused. Then verified everything *around* that one line — platform/error/size/extension checks, refuse-on-zero-valid-rows, the exact write path, re-reading the just-uploaded file back through the real `CsvAdSpendProvider`, an untouched sibling platform's file — using PHP's standard technique for this exact situation: an unqualified function call resolves first against the caller's own namespace, so a same-named `is_uploaded_file()` defined in `Aavirbhava\AdsAnalytics\Model\AdSpendProvider` intercepts only that one call site. Kept in its own file (`Test/Unit/Model/AdSpendProvider/_files/is_uploaded_file_override.php`), not declared inline via bracketed `namespace X { ... }` blocks in the test file itself — both are valid PHP and both were tried, but phpcs's tokenizer does not parse the bracketed multi-namespace form cleanly (a real, reproducible phpcs limitation, not a code defect — `php -l` and a full green test run both confirmed the bracketed form was syntactically and behaviourally correct before it was moved out purely to keep phpcs clean).

**Rendered the real admin template both ways.** With the ACL gate closed (no real admin session in a raw bootstrap script — the same "fails closed without a session" behaviour already established for other ACL checks this session): the whole upload section is correctly absent, and every other section (charts, ROAS table, PDF link) still renders. With the gate forced open (a one-method test subclass, since the gate itself was already independently confirmed): the section, both platform rows with real row counts and timestamps, the form's POST target with a real admin security key, the CSRF `form_key` hidden field, the `platform_code` hidden field and the file input are all present and correct.

21 new/updated unit tests (233 total green), 0 phpcs errors. The two phpstan findings this introduces (`RequestInterface::getFiles()` not on the interface) are confirmed not specific to this code — running phpstan against core's own `Magento_TaxImportExport\Controller\Adminhtml\Rate\ImportPost` (the only other core controller found calling `getFiles()` the same way) produces the identical finding.
...
### Admin config moved under the shared "Aavirbhava" tab, and ad-spend data made discoverable (2026-09-20)
Two changes, both requested directly.

**The section moved from the stock "Services" tab to "Aavirbhava"**, the tab `Aavirbhava_SalesAnalytics` already declares in its own `system.xml`. Not redeclared here — Magento merges `<tab>` nodes by id across every module, so a second module referencing an existing tab id just needs `<tab>aavirbhava</tab>` on its section, not a second `<tab id="aavirbhava">` node. `sortOrder="20"` places it after Sales Analytics (10) in that tab, and `class="separator-top"` matches the visual convention Sales Analytics already uses. Verified via `Structure::getElement('aavirbhava_adsanalytics')->getData()`: `tab => aavirbhava`, `sortOrder => 20`, `class => separator-top`.

**A new read-only "Ad Spend Data (ROAS)" field explains where `CsvAdSpendProvider` reads from and in what format**, because there was previously no way for an admin to discover the file path or CSV columns without reading source code — a real gap, raised directly. It documents: the exact path (`var/aavirbhava/adsanalytics/adspend/<platform_code>.csv`), the `date,campaign,spend` format with an example row, which platforms are wired by default (`google`, `meta`), that a missing file means "no data" rather than an error, and that this is a reference implementation a real API-backed provider can replace via `Api\AdSpendProviderInterface` without touching this module's core.

There is nothing to save on this field — the value comes from a file, not configuration — so it uses `type="label"` with a `frontend_model` (`Block\Adminhtml\System\Config\AdSpendNote`) that renders the label and comment as static text instead of an input box. This is not a new UI pattern: it is structurally identical to core's own `Magento\Analytics\Block\Adminhtml\System\Config\AdditionalComment`, not reused directly only because doing so would pull in a dependency on `Magento_Analytics` for a single ~10-line method. Confirmed the two phpstan findings this produces (`AbstractElement::getLabel()`/`getComment()` "undefined" — both are `DataObject` magic accessors phpstan cannot resolve without Magento's own phpstan extension) are not specific to this code: running phpstan against core's `AdditionalComment.php` directly produces the identical two findings, proving it is a known gap in scanning Magento's dynamic accessor pattern, not a defect.

0 phpcs errors, 211 unit tests still green (no test-relevant logic changed).
